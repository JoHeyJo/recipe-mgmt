...
/** Empty item name validation -  if another part of the ingredient amount or unit is populated
 * find empty item.name field and throw error */
function checkForEmptyItemInput(ingredients: Ingredients) {
  const isEmpty = ingredients.some(
    (part) =>
      part.item.id === null && (part.amount.id !== null || part.unit.id !== null),
  );
...
function filterInstructions(original: Instructions, edited: Instructions) {
  const alteredInstructions = edited.reduce(
  ...
  const alteredIngredients = edited.reduce(
    (alteredIngredients, editedIngredient, index) => {
      ...
      if (isInputModified) {
        ...
        const isIngredientModified =
          isAmountModified || isUnitModified || isItemModified;
